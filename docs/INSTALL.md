...
**Success indicators:** - ✅ Package loads without errors - ✅ Data
loads successfully  
- ✅ Zoom function works and shows a plot - ✅ 5-panel plot generates
without errors
...
If you need to process your own GTF files or create new data objects,
you’ll also need:
...
**Note**: These Bioconductor packages are NOT required for normal
package usage.
...
1.  **Package not found**: Make sure you’re using the correct repository
    URL
2.  **VCF file reading errors**: Make sure `vcfR` is properly installed
3.  **Plot combination issues**: Ensure `patchwork` is installed
4.  **Data manipulation errors**: Verify `dplyr` and `tidyr` are up to
    date
